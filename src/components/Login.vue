...
export default {
  ...
  methods: {
    ...
    async handlePhoneLogin() {
      ...
      try {
        ...
        if (result.code === 200) {
          ...
          // 使用认证存储来获取完整用户信息
          this.authStore.fetchCompleteUserInfo().then(async () => {
            // 检查是否是管理员账号
            const isAdmin = await this.checkAdminAccess();
    ...
    async handleWechatLoginSuccess(data) {
      ...
      // 使用认证存储来获取完整用户信息
      await this.authStore.fetchCompleteUserInfo();
      
      // 检查是否是管理员账号（使用用户名，因为微信登录会返回用户名）
      let isAdmin = false;
      if (user && user.username) {
        this.formData.username = user.username;
        this.formData.password = ''; // 微信登录没有密码，传空字符串
        isAdmin = await this.checkAdminAccess();
      }
    ...
    async checkAdminAccess() {
      try {
        // 尝试使用管理员登录接口验证
        const response = await userAPI.checkAdminAccess(this.formData.username, this.formData.password);
        
        if (response.code === 200 && response.data.isAdmin) {
          // 保存管理员信息
          localStorage.setItem('admin_token', localStorage.getItem('token'));
          localStorage.setItem('admin_info', JSON.stringify({
            id: response.data.adminId || this.authStore.userInfo.id,
            username: this.formData.username,
            realName: response.data.realName || this.formData.username,
            roleId: response.data.roleId || 1
          }));
          console.log('✅ 检测到管理员账号，已自动保存管理员信息');
          return true;
        }
      } catch (error) {
        // 如果不是管理员，忽略错误
        console.log('ℹ️ 普通用户账号，无需保存管理员信息');
      }
      return false;
    },
  },
