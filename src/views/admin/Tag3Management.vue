<template>
    <div class="tag3-management">
        <el-card>
            <template #header>
                <div class="card-header">
                    <span>标签管理（三级标签 - 地点实体）</span>
...
const loadData = async () => {
    ...
    try {
        ...
        const res = await tagAPI.getLevel3Tags(params)
...
const handleToggleStatus = async (row) => {
    ...
    try {
        ...
        const res = await tagAPI.updateLevel3TagStatus(row.id, newStatus)
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该标签吗？', '警告', {
        ...
        try {
            const res = await tagAPI.deleteLevel3Tag(row.id)
            if (res.code === 200 || res.success) {
