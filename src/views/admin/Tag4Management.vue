<template>
    <div class="tag4-management">
        <el-card>
            <template #header>
                <div class="card-header">
                    <span>标签管理（四级标签 - 自定义标签）</span>
...
const loadData = async () => {
    ...
    try {
        ...
        const res = await tagAPI.getLevel4Tags(params)
...
const handleToggleStatus = async (row, status) => {
    ...
    try {
        ...
        const res = await tagAPI.updateLevel4TagStatus(row.id, status)
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定要删除该标签吗？此操作不可恢复！', '警告', {
        ...
        try {
            const res = await tagAPI.deleteLevel4Tag(row.id)
            if (res.code === 200 || res.success) {
...
const handleSubmit = async () => {
    ...
    await formRef.value.validate(async (valid) => {
        ...
        try {
            const res = await tagAPI.updateLevel4Tag(formData.id, formData)
